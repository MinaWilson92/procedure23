...
const AdminDashboard = ({ procedures, onDataRefresh, sharePointAvailable }) => {
  ...
  // FIX #1: Configure the PnPjs library with the correct site URL once, when the component loads.
  useEffect(() => {
  ...
  const loadDashboardData = async () => {
    ...
    setError(null);
    try {
      // FIX #2: Use the simple 'select *' query that matches the user's list columns.
      const sp = window.pnp.sp.web.lists.getByTitle('Procedures');
      const items = await sp.items.select('*').get();
      
      console.log("Procedures loaded successfully. Sample item:", items[0]); // For debugging

      setAllProcedures(items);
      processProcedures(items);

      // This is placeholder data for the User Management tab, can be adapted later.
      const dummyUsers = [
  ...
  const processProcedures = (proceduresData) => {
    ...
    proceduresData.forEach(p => {
      const created = new Date(p.Created); // 'Created' is a default SharePoint field
      const expiry = p.ExpiryDate ? new Date(p.ExpiryDate) : null; // Assumes 'ExpiryDate' field exists
      ...
      if (p.Status === 'Active') { // Assumes 'Status' field exists
        activeCount++;
  ...
  // FIX #3: Updated Add/Edit functions to match the list schema (e.g., PrimaryOwner)
      ...
      >
              ...
              <Paper elevation={0} sx={{ p: 3, bgcolor: 'background.paper' }}>
                ...
                {expiringSoon.length > 0 ? (
                  <List>
                    {expiringSoon.map(p => (
                      <ListItem key={p.ID} secondaryAction={
                        <Chip label={`Expires: ${p.ExpiryDate ? new Date(p.ExpiryDate).toLocaleDateString() : 'N/A'}`} color="warning" size="small" />
                      }>
                        <ListItemText
                          primary={p.Title}
              ...
              <Paper elevation={0} sx={{ p: 3, bgcolor: 'background.paper' }}>
                ...
                {overdue.length > 0 ? (
                  <List>
                    {overdue.map(p => (
                      <ListItem key={p.ID} secondaryAction={
                        <Chip label={`Expired: ${p.ExpiryDate ? new Date(p.ExpiryDate).toLocaleDateString() : 'N/A'}`} color="error" size="small" />
                      }>
                        <ListItemText
                          primary={p.Title}
                          secondary={`Status: ${p.Status}`}
              ...
              <TableBody>
                {filteredProcedures.map((p) => (
                  <TableRow key={p.ID}>
                    ...
                    <TableCell>{p.PrimaryOwner || 'N/A'}</TableCell>
                    <TableCell>{p.SecondaryOwner || 'N/A'}</TableCell>
                    <TableCell>
        ...
        <TabPanel value={activeTab} index={2}>
          ...
          <Paper elevation={0} sx={{ p: 3, bgcolor: 'background.paper' }}>
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <TextField fullWidth label="Title" value={newProcedure.Title} onChange={(e) => setNewProcedure({ ...newProcedure, Title: e.target.value })} />
              </Grid>
              <Grid item xs={12}>
                <TextField fullWidth label="Description" multiline rows={4} value={newProcedure.Description} onChange={(e) => setNewProcedure({ ...newProcedure, Description: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Category" value={newProcedure.Category} onChange={(e) => setNewProcedure({ ...newProcedure, Category: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Tags (comma-separated)" value={newProcedure.Tags} onChange={(e) => setNewProcedure({ ...newProcedure, Tags: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Primary Owner" value={newProcedure.PrimaryOwner} onChange={(e) => setNewProcedure({ ...newProcedure, PrimaryOwner: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Secondary Owner" value={newProcedure.SecondaryOwner} onChange={(e) => setNewProcedure({ ...newProcedure, SecondaryOwner: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Expiry Date" type="date" InputLabelProps={{ shrink: true }} value={newProcedure.ExpiryDate || ''} onChange={(e) => setNewProcedure({ ...newProcedure, ExpiryDate: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <FormControl fullWidth>
                  <InputLabel>Status</InputLabel>
                  <Select value={newProcedure.Status} label="Status" onChange={(e) => setNewProcedure({ ...newProcedure, Status: e.target.value })} >
                    <MenuItem value="Draft">Draft</MenuItem>
                    <MenuItem value="Pending Review">Pending Review</MenuItem>
                    <MenuItem value="Active">Active</MenuItem>
                    <MenuItem value="Archived">Archived</MenuItem>
              ...
              <Grid item xs={12}>
                <Button variant="contained" color="primary" startIcon={loading ? <CircularProgress size={20} /> : <Add />} onClick={handleAddProcedure} disabled={loading || !newProcedure.Title} >
            ...
            <Box mb={3}>
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} sm={6} md={5}>
                  <Button variant="contained" color="primary" startIcon={<Save />} onClick={handleUpdateUserRole} disabled={loading || !selectedUser || !selectedRole} sx={{ mr: 1 }} >
                    Update Role
                  </Button>
                  <Button variant="contained" color="success" startIcon={<PersonAdd />} onClick={handleGrantAccess} disabled={loading || !selectedUser} sx={{ mr: 1 }} >
                    Grant Access
                  </Button>
                  <Button variant="contained" color="error" startIcon={<Delete />} onClick={handleRevokeAccess} disabled={loading || !selectedUser} >
                    Revoke Access
                  </Button>
                </Grid>
              </Grid>
                ...
                <TableBody>
                  {users.map(user => (
                    <TableRow key={user.id}>
                      ...
                      <TableCell>
                        <Chip label={user.role} color={user.role === 'Admin' ? 'primary' : user.role === 'Reviewer' ? 'info' : 'default'} size="small" />
                      </TableCell>
                      <TableCell>
