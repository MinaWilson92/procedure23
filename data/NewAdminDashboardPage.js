...
const AdminDashboard = ({ procedures, onDataRefresh, sharePointAvailable }) => {
  ...
  // ✅ NEW: Start email monitoring when admin dashboard loads
  useEffect(() => {
    if (sharePointAvailable && isAdmin) {
      console.log('🚀 Starting automated email monitoring...');
  ...
  // ✅ ENHANCED: Handle Grant Access with email notification
    ...
    try {
      ...
      if (sharePointAvailable) {
        ...
        if (response.ok) {
          ...
          try {
            await emailNotificationService.triggerUserAccessNotification(
              newUser.userId, 
              newUser.displayName,
            ...
            setNotification({ 
              type: 'success', 
              message: `✅ Access granted to ${newUser.displayName} - Welcome email sent successfully` 
            });
          } catch (emailError) {
            console.warn('⚠️ Access granted but email notification failed:', emailError);
            setNotification({ 
              type: 'warning', 
              message: `✅ Access granted to ${newUser.displayName}, but email notification failed: ${emailError.message}` 
            });
          }
 ...
 // ✅ ENHANCED: Edit User Role with email notification
    ...
    try {
      ...
      if (sharePointAvailable) {
        ...
        if (response.ok || response.status === 204) {
          ...
          // ✅ NEW: Trigger role change email notification
          try {
            await emailNotificationService.triggerUserRoleChangeNotification(
              userRole.userId,
              userRole.displayName,
              userRole.role,
              newRole,
            ...
            setNotification({ 
              type: 'success', 
              message: `✅ Role updated to ${newRole} for ${userRole.displayName} - Notification email sent` 
            });
          } catch (emailError) {
            console.warn('⚠️ Role updated but email notification failed:', emailError);
            setNotification({ 
              type: 'warning', 
              message: `✅ Role updated to ${newRole} for ${userRole.displayName}, but email notification failed: ${emailError.message}` 
            });
          }
...
// ✅ ENHANCED: Revoke Access with email notification
  ...
  if (window.confirm(`Are you sure you want to revoke access for ${userRole.displayName} (${userRole.userId})?`)) {
    try {
      ...
      if (sharePointAvailable) {
        ...
        // ✅ NEW: Trigger access revoked email notification
        try {
          await emailNotificationService.triggerUserAccessRevokedNotification(
            userRole.userId,
            userRole.displayName,
            user?.displayName || user?.staffId,
          ...
          console.log('✅ Access revoked notification email sent');
        } catch (emailError) {
          console.warn('⚠️ Access revoked but email notification failed:', emailError);
        }
        ...
        if (response.ok || response.status === 204) {
          ...
          setNotification({ 
            type: 'success', 
            message: `✅ Access revoked for ${userRole.displayName} - Notification email sent` 
          });
          loadUserRoles();
        } else {
          const errorText = await response.text();
...
return (
  <Container maxWidth="lg" sx={{ py: 4 }}>
    {/* ✅ Header with Email Monitoring Status */}
    <Box sx={{ mb: 4 }}>
      ...
      <Typography variant="body1" color="text.secondary">
        Comprehensive administration and management tools with automated email notifications
      </Typography>
      ...
      <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
        ...
        <Chip 
          ...
          icon={<Person />}
        />
        {/* ✅ NEW: Email monitoring status indicator */}
        <Chip 
          icon={<Email />}
          label="Email Monitoring Active"
          color="success"
          variant="outlined"
        />
    ...
    {/* ✅ NEW: Email Monitoring Status Alert */}
    {sharePointAvailable && (
      <Alert severity="success" sx={{ mb: 3 }}>
        <Typography variant="body2">
          <strong>📧 Automated Email Monitoring:</strong> System automatically checks for procedure expiry every 24 hours. 
          Email notifications are sent for user access changes, procedure uploads, and expiry alerts.
        </Typography>
      </Alert>
    )}
            ...
            <List>
              <ListItem button onClick={() => navigate('admin-panel')}>
                ...
                <ListItemText 
                  ...
                  secondary="Add new procedure document with email notifications"
                />
              </ListItem>
              ...
              <ListItem button onClick={() => setActiveTab(2)}>
                ...
                <ListItemText 
                  ...
                  secondary="User roles & permissions with email alerts"
                />
              </ListItem>
    ...
    <Card>
          ...
          {sharePointAvailable && procedures && procedures.length > 0 ? (
            <Alert severity="success" sx={{ mb: 3 }}>
              <Typography variant="body2">
                ...
                Quality scores and metrics are calculated from actual procedure data. Email monitoring is active.
              </Typography>
            </Alert>
          ) : (
            <Alert severity="warning" sx={{ mb: 3 }}>
              <Typography variant="body2">
                ...
                Connect to SharePoint to see real procedure analytics and enable email notifications.
              </Typography>
            </Alert>
          )}
      ...
      {activeTab === 1 && (
        <Box sx={{ p: 3 }}>
          ...
          {/* ✅ NEW: Email notification info for procedures */}
          <Alert severity="info" sx={{ mb: 3 }}>
            <Typography variant="body2">
              📧 <strong>Email Notifications:</strong> When procedures are uploaded, automatic notifications are sent to configured recipients. 
              The system also monitors expiry dates and sends alerts at 30 days, 7 days, and when expired.
            </Typography>
          </Alert>
      ...
      {activeTab === 2 && (
        <Box sx={{ p: 3 }}>
          ...
          {/* ✅ ENHANCED: Email notification info for user management */}
          <Alert severity="info" sx={{ mb: 3 }}>
            <Typography variant="body2">
              <strong>📧 Email Notifications:</strong> When user access is granted, revoked, or roles are changed, automatic email notifications are sent to:
              <br />• The affected user
              <br />• All configured admin recipients
              <br />• System audit trail is maintained with full logging
              {sharePointAvailable && (
        ...
        <Box sx={{ p: 3 }}>
          ...
          <Alert severity="info" sx={{ mb: 3 }}>
            ...
            {sharePointAvailable ? (
              <Typography variant="body2" sx={{ mt: 1 }}>
                <strong>Live Data:</strong> Real audit logs from SharePoint Lists including email notification tracking.
              </Typography>
            ) : (
              <Typography variant="body2" sx={{ mt: 1 }}>
          ...
          <List>
            ...
            <ListItem>
              ...
              <ListItemText 
                primary="Email monitoring active"
                secondary="Automated notifications for expiry, access changes, and uploads"
              />
            </ListItem>
          </List>
    ...
    <Dialog open={accessDialog.open} onClose={() => setAccessDialog({ open: false })}>
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          Grant User Access
          {sharePointAvailable && (
            <Chip label="SharePoint + Auto-fetch + Email" size="small" color="success" variant="outlined" />
          )}
        </Box>
      </DialogTitle>
      <DialogContent>
        <Box sx={{ pt: 1 }}>
          ...
          {sharePointAvailable && (
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="caption">
                ✅ User will be added to UserRoles list<br/>
                ✅ Action logged to AccessAuditLog<br/>
                ✅ Welcome email sent automatically<br/>
      ...
      <DialogActions>
        ...
        >
          {loading ? 'Granting...' : 'Grant Access & Send Email'}
        </Button>
      </DialogActions>
    </Dialog>
