...
const ProfessionalCard = styled(Card)(({ theme, accent = false }) => ({
  background: accent ? 
...
const ProfessionalLogo = styled(Box)(({ theme, size = 40 }) => ({
  width: size,
  height: size,
...
const HSBCProceduresHub = () => {
  ...
  return (
    ...
    <ProfessionalAppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1 }}>
        <Toolbar>
          <IconButton
            color="inherit"
            edge="start"
...
<ProfessionalLogo size={40}>
  ...
  sx={{
    ml: 2,
    borderColor: sharePointAvailable ? '#4caf50' : '#ff9800',
    color: sharePointAvailable ? '#4caf50' : '#ff9800',
