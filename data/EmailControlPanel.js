// components/email/EmailControlPanel.js - FIXED VERSION
...
const EmailControlPanel = ({ user, emailService }) => {
  // ✅ FIXED: Proper service initialization with correct method bindings
  const [testingService] = useState(() => {
    try {
      return {
        ...
        // ✅ FIXED: Check templates method with proper baseUrl
        checkAvailableTemplates: async () => {
          try {
            console.log('🔍 Checking available email templates...');
            
            const baseUrl = 'https://teams.global.hsbc/sites/EmployeeEng';
            const response = await fetch(
              `${baseUrl}/_api/web/lists/getbytitle('EmailTemplates')/items?$select=Id,Title,TemplateType,IsActive,Subject,Created,Modified&$orderby=TemplateType`,
              {
                headers: { 
            ...
            if (!response.ok) {
              ...
              return {
                ...
                inactiveTemplates: [],
                templatesByType: {},
                error: `HTTP ${response.status}: Cannot access EmailTemplates list`
              };
            }
        ...
        // ✅ FIXED: Quick notification test using emailService directly
        quickTestNotification: async (notificationType, user) => {
          ...
          try {
            ...
            switch (notificationType) {
              case 'new-procedure-uploaded':
                console.log('📧 Testing New Procedure Upload notification...');
                
                if (!emailService.triggerProcedureUploadNotification) {
                  return { success: false, message: 'triggerProcedureUploadNotification method not available' };
                }
                ...
                const mockUploadResult = {
                  ...
                  procedure: {
                    name: 'QUICK TEST: New Procedure Upload Notification',
                    lob: 'TEST',
                    primary_owner: user?.displayName || 'Test User',
                    primary_owner_email: testRecipient,
                    score: 88,
                    uploaded_by_name: user?.displayName || 'Quick Test System'
                  },
                  analysisResult: {
                    score: 88,
                    details: { quickTest: true }
                  }
                };
                
                const uploadResult = await emailService.triggerProcedureUploadNotification(mockUploadResult);
                return uploadResult;

              case 'user-role-updated':
                console.log('📧 Testing User Role Update notification...');
                
                if (!emailService.triggerUserChangeNotification) {
                  return { success: false, message: 'triggerUserChangeNotification method not available' };
                }
                ...
                const mockLogEntry = {
                  ...
                  TargetUserName: user?.displayName || 'Test User',
                  TargetUserId: user?.staffId || '43898931',
                  PerformedByName: 'Quick Test System',
                  OldValue: 'User',
                  NewValue: 'Admin',
                  LogTimestamp: new Date().toISOString(),
                  Details: 'Quick test of user role change notification',
                  Reason: 'Testing email functionality'
                };
                
                const userResult = await emailService.triggerUserChangeNotification(testRecipient, mockLogEntry);
                return userResult;

              default:
                return { success: false, message: `Unknown notification type: ${notificationType}` };
            }
        ...
        // ✅ FIXED: Quick configuration test
      ...
      return {
        ...
        getWeeklyStatistics: async () => {
          ...
          try {
            ...
            return {
              weekPeriod: {
                start: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).​​​​​​​​​​​​​​​​               end: new Date().toISOString()
              },
              emailActivity: {
                total: 0,
  ...
  // ✅ FIXED: Quick test notification function
  const quickTestNotification = async (notificationType) => {
    try {
      ...
      console.log(`🧪 Starting quick test for: ${notificationType}`);
      const result = await testingService.quickTestNotification(notificationType, user);
      
      if (result.success) {
        alert(`✅ ${notificationType} test successful! Check your email inbox.`);
      } else {
  ...
  // ✅ FIXED: Quick test configuration function
  const quickTestConfiguration = async () => {
    try {
      ...
      const result = await testingService.quickTestConfiguration();
      
      if (result.success) {
        alert('✅ Configuration test successful! Check your email inbox.');
      } else {
  ...
  // ✅ FIXED: Check templates function
  const checkAvailableTemplates = async () => {
    try {
      ...
      console.log('🔍 Checking available email templates...');
      const templatesData = await testingService.checkAvailableTemplates();
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6} md={4}>
                <Button
                  ...
                  variant="outlined"
                  startIcon={<Email />}
                  onClick={quickTestConfiguration}
                  disabled={loading}
                  color="info"
                >
                  Test Configuration
              ...
              <Grid item xs={12} sm={6} md={4}>
                <Button
                  ...
                  variant="outlined"
                  startIcon={<Info />}
                  onClick={checkAvailableTemplates}
                  disabled={loading}
                  color="info"
                >
                  🔍 Check Templates
